...
export const alt = "aujezus.com portfolio website";
export const size = {
  width: 1200,
  height: 630,
};
...
export async function Image() {
  const fontRobotoMonoMedium = await fetch(
    new URL("../../assets/fonts/RobotoMono-Medium.ttf", import.meta.url),
  ).then((res) => res.arrayBuffer());
  const fontRobotoMonoRegular = await fetch(
    ...
    (
      <div tw="w-full h-full bg-[rgb(3,7,18)] p-20 flex text-[rgb(249,250,251)] flex-col justify-between">
        <div tw="text-2xl font-semibold flex">
          AUJEZUS <span tw="ml-2 text-[rgb(109,40,217)]">home_</span>
        </div>

        <div tw="flex flex-col items-center">
          <div tw="text-[150px] font-semibold">AUJEZUS</div>

          <div tw="text-2xl flex">
            <span tw="text-[rgb(109,40,217)] mr-2">{">"}</span> Augustas Vaivada
        ...
        <div tw="flex justify-around">
          ...
          <div tw="flex items-center">
            ...
            <span tw="ml-2">github.com/AuJezus</span>
          </div>
        </div>
      </div>
    ),
